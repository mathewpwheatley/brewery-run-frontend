...
const CircuitMap = ({
      ...
      if (status === 'OK') {
        ...
        const totalDistance =
          response.routes[0].legs.reduce(
            (sum, current) => sum + current.distance.value,
            0
          ) / 1603.34
        ...
        elevationService.getElevationAlongPath(
          { path: response.routes[0].overview_path, samples: sampleCount },
          (elevationData, status) => {
            if (status !== 'OK') {
              window.alert(
  ...
  const plotElevation = (elevationData, sampleCount, totalDistance) => {
    ...
    const elevation = elevationData.map((elevation) => {
      ...
      let currentElevation =
        Math.round((elevation.elevation / 0.3048) * 100) / 100
    ...
    if (
      elevationGain !== storedElevation ||
      totalDistance.toString() !== storedDistance
    ) {
      updateDistanceElevationCircuit(
        circuitId,
    ...
    new Chart(googleMapBottomPanel, {
      ...
      options: {
        ...
        tooltips: {
          ...
          callbacks: {
            title: (tooltipItem) => {
              return 'Miles: ' + tooltipItem[0].xLabel
            },
          },
        },
        scales: {
  ...
  const [showDirections, setShowDirections] = useState(
    hideDirectionsDefault ? false : true
  )
  ...
  return (
    <Card className='p-0' style={{ width: '100%', height: '100%' }}>
      ...
      <CardGroup style={mapSize ? mapSize : { width: '100%', height: '45vh' }}>
        ...
        />
        <Card
          className='col-4 m-0 p-0 rounded-0'
          style={{
            maxHeight: '100%',
            overflowY: 'scroll',
      ...
      <Card className='m-0 rounded-0 rounded-bottom' style={{ height: '15vh' }}>
...
const mapDispatchToProps = (dispatch) => {
  return {
    updateDistanceElevationCircuit: (circuitId, distance, elevation) => {
      dispatch(updateDistanceElevationCircuit(circuitId, distance, elevation))
    },
    addCircuitLegs: (circuit_legs) => {
      dispatch(addCircuitLegs(circuit_legs))
    },
  }
}
